...
class Edit extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        if ($id) {
            $model->load($id);
            if (!$model->getId()) {
                $this->messageManager->addError(__('This categoryicon no longer exists.'));
        ...
        $this->_coreRegistry->register('categoryicon', $model);

        // 5. Build edit form
		/** @var \Magento\Backend\Model\View\Result\Page $resultPage */
        $resultPage = $this->_initAction();
        $resultPage->addBreadcrumb(
            $id ? __('Edit Categoryicon') : __('New Categoryicon'),
            $id ? __('Edit Categoryicon') : __('New Categoryicon')
        );
        $resultPage->getConfig()->getTitle()->prepend(__('Categoryicon'));
        $resultPage->getConfig()->getTitle()
            ->prepend($model->getId() ? $model->getTitle() : __('New Categoryicon'));
        return $resultPage;
    }
}
