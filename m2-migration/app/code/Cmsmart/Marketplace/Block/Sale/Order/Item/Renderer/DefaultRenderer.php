...
namespace Cmsmart\Marketplace\Block\Sale\Order\Item\Renderer;
...
class DefaultRenderer extends \Magento\Framework\View\Element\Template
{
    ...
    public function __construct(
        ...
        \Cmsmart\Marketplace\Model\ProductFactory $sellerProductFactory,
        ...
        $this->_sellerFactory = $sellerProductFactory;
    ...
    public function getSellerProductIds() {
        $sellerId = $this->_registry->registry('seller_id');
        if (!($sellerId)) {
            return array();
        }

        $sellerCollection = $this->_sellerFactory->create()->getCollection()->addFieldToFilter('seller_id', $sellerId);
        $productIds = array();
    ...
    public function getSellerData () {
        ...
        $sellerCollection = $this->_sellerDataFactory->create()->getCollection()->addFieldToFilter('seller_id', $sellerId);
        $sellerData = array();
        foreach ($sellerCollection as $item) {
            array_push($sellerData, $item->getData());
        }
